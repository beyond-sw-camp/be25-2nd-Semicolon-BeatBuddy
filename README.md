...
#### 방지혁
> 인증 도메인을 처음부터 설계하면서, 단순히 "로그인/로그아웃"이 아니라 생각해야 할 게 훨씬 많다는 걸 깨달았습니다. Access Token은 짧게, Refresh Token은 Redis에 저장해 관리하는 구조를 직접 구현했고, HttpOnly 쿠키에 담는 이유도 처음엔 "왜 굳이?" 싶었는데 XSS 공격으로부터 토큰을 보호하기 위한 것임을 이해하고 나서 납득이 되었습니다. Spring Security가 Filter 레이어에서 JWT를 검증하고, 비즈니스 예외는 @ControllerAdvice에서 처리하는 역할 분리도 처음엔 중복처럼 보였지만, 두 레이어의 책임이 애초에 다르다는 걸 이해하고 나니 구조가 훨씬 명확하게 보였습니다.
